<?php
/**
 * A workflow action that notifies sends a notification email to one email address, not necessary
 ...
 * @subpackage actions
 */
class NotifyByEmailWorkflowAction extends WorkflowAction {
	...
	public function getCMSFields() {
		...
		$fields->addFieldsToTab('Root.Main', array(
			...
			new TextField('EmailTo', $this->fieldLabel('EmailTo')),
	...
	public function fieldLabels($relations = true) {
		return array_merge(parent::fieldLabels($relations), array(
			...
			'NotificationNote'  => _t('NotifyUsersWorkflowAction.NOTIFICATIONBYEMAILNOTE',
				'Will send a notification email to the address in the \'Email to\' field'),
			'EmailTo' => _t('NotifyUsersWorkflowAction.EmailTo', 'Email to'),
	...
	public function execute(WorkflowInstance $workflow) {
		...
		$to = str_replace(array_keys($variables), array_values($variables), $this->EmailTo);
