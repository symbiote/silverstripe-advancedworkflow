				...
				switch(!0) {
					...
					case (!s.data && !!s.ajax) || (!!s.data && !!s.ajax && obj && obj !== -1):
						...
						success_func = function (d, t, x) {
							var sf = self.get_settings().json_data.ajax.success; 
							if(sf) { d = sf.call(self,d,t,x) || d; }
							if(d === "" || (!$.isArray(d) && !$.isPlainObject(d))) {
								return error_func.call(self, x, t, "");
							}
							d = self._parse_json(d);
							if(d) {
								if(obj === -1 || !obj) { self.get_container().children("ul").empty().append(d.children()); }
								else { obj.append(d).children(".jstree-loading").removeClass("jstree-loading"); obj.data("jstree-is-loading",false); }
								self.clean_node(obj);
								if(s_call) { s_call.call(self); }
							}
							else {
								if(obj === -1 || !obj) {
									if(s.correct_state) { 
										self.get_container().children("ul").empty(); 
										if(s_call) { s_call.call(self); }
									}
								}
								else {
